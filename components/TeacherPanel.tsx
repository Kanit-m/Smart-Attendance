...
import { 
  ...
  Building2, RotateCcw, History
} from 'lucide-react';
import { 
  collection, getDocs, query, where, writeBatch, doc
} from 'firebase/firestore/lite';
import { db } from '../firebase';
import { Student, AttendanceStatus, Holiday, Role, Gender } from '../types';
...
export const TeacherPanel: React.FC<TeacherPanelProps> = ({ currentUser, onBackToAdmin }) => {
  ...
  const [historyData, setHistoryData] = useState<{
      ...
      stats: Record<string, { present: number; absent: number; late: number; sick: number; personal: number }>;
  } | null>(null);
  ...
  const fetchHistory = async () => {
      ...
      try {
          ...
          studentsList.forEach(s => {
              records[s.id] = {};
              studentStats[s.id] = { present: 0, absent: 0, late: 0, sick: 0, personal: 0 };
          });
